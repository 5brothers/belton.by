	...
	public function fenom($chunk, array $properties = array()) {
		...
		if ($fenom = $this->getFenom()) {
			...
			if ($tpl instanceof Fenom\Render) {
				...
				$properties['_config'] = $this->modx->config;
				$objects = array(
					'context' => 'key',
					'resource' => 'id',
					'user' => 'id',
				...
				foreach ($objects as $k => $v) {
					if (!$item = $this->getStore($this->modx->$k->$v, "fenom_{$k}")) {
						$item = $this->modx->$k->toArray();
						if ($k == 'user') {
							$item = array_merge($this->modx->user->Profile->toArray(), $item);
						}
						elseif ($k == 'resource') {
							if ($tvs = $this->modx->resource->getTemplateVars()) {
								/** @var modTemplateVar $tv */
								foreach ($tvs as $tv) {
									$tvname = $tv->get('name');
									$item[$tvname] = $this->modx->resource->getTVValue($tvname);
								}
							}
						}
						$this->setStore($this->modx->$k->$v, $item, "fenom_{$k}");
					}
					$properties["_{$k}"] = $item;
				}
				// Add system objects
		...
		switch ($method) {
			case 'lexicon':
				...
				if (!empty($params[0]) && is_array($params[0])) {
					$properties = $params[0];
					$topic = !empty($properties['topic'])
						? $properties['topic']
						: 'default';
					$namespace = !empty($properties['namespace'])
						? $properties['namespace']
						: 'core';
					$language = !empty($properties['language'])
						? $properties['language']
						: $this->modx->getOption('cultureKey', null, 'en');
					$this->modx->lexicon->load($language . ':' . $namespace . ':' . $topic);
				}
				$placeholders = !empty($params[1]) && is_array($params[1])
					? $params[1]
					: array();
			...
			case 'url':
				...
				if (is_numeric($id)) {
					...
					$args = !empty($params[1]) && is_array($params[1])
						? $params[1]
						: array();
