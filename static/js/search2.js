...
var searchAsYouTypeConfiguration = {
	...
	//количество букв, с которого начинаются подсказки
    minLen: 4
  };
 ...
 */
SearchAsYouType.prototype.initialize = function(inputFieldEl, focus, host) {
	
	searchAsYouTypeConfiguration.ajaxResponderUrl = host;
...
SearchAsYouType.prototype.createDomElements_ = function() {
  ...
  */
  ...
  /* не будем добавлять картинку loading.gif
...
SearchAsYouType.prototype.updateDimensionsAndShadow_ = 
  ...
  // right
  // (...) height too
  /* не будем изменять положение картинки loading.gif 
...
SearchAsYouType.prototype.search_ = function(dontDelayShowResults) {
  ...
  // If a query is smaller than threshold we don't do anything

  if (this.typedQuery.length < searchAsYouTypeConfiguration.minLen || this.typedQuery.length == 0) {
    this.changeWaitingForSearchResults_(false);
    return;
  }

  URL = searchAsYouTypeConfiguration.ajaxResponderUrl;
  URL += "?q=" + encodeURIComponent(this.typedQuery);
  URL += "&format=rich";
  URL += "&max=" + encodeURIComponent(searchAsYouTypeConfiguration.maxFullResults);
  URL += "&callback=searchAsYouType.handleAjaxResponse";
  if (this.debugMode) {
    URL += "&debug=1";
  }
...
SearchAsYouType.prototype.changeWaitingForSearchResults_ = function(value) {
  if (this.waitingForSearchResults != value) {
    if (value) {
      /* не будем показывать картинку loading.gif
	  this.waitingForSearchResultsEl.style.visibility = 'visible';
	  */
    } else {
	  /* не будем скрывать картинку loading.gif	
      this.waitingForSearchResultsEl.style.visibility = 'hidden';
	  */
...
/*  if (results.results.length && this.tipText) { 
  ...
  for(var i = 0; i < results.results.length; i++){
	if (typeof(results.results[i].style) == 'undefined'){
		results.results[i].style="normal";
		// results.results[i].moreDetailsUrl = document.location.pathname + "?q=" + results.results[i].name;
		
		// this.inputFieldEl.parentNode.parentNode.action = значению атрибута action у родительской формы элемента input
		// верстка имеет структуру <form ...><span ...><input ... /></span></form> , то есть форма - родительский элемент родительского элемента input
		// this.inputFieldEl - поле с поиском, parentNode - родительский элемент, action - значение атрибута "action"
		results.results[i].moreDetailsUrl = this.inputFieldEl.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.action + "?q="
        + encodeURIComponent(results.results[i].name);

		
		// results.results[i].content = results.results[i].name;
		results.results[i].content = results.results[i].name.replace(/<\/?[^>]+>/gi, '');
	}
  }
...
SearchAsYouType.prototype.processResults_ = function(results, cached) {
  ...
  // come out of order)
  if (results.query.length == this.typedQuery.length) {
...
SearchAsYouType.prototype.prepareResultsWindow_ = function() {
  ...
  if (this.results.countNotCompact <= 
      searchAsYouTypeConfiguration.maxNotCompactResults) {
    for (var i = 0; i < this.results.results.length; i++) {
    ...
    case 32: // space
      if ((!this.resultsWindowHidden) && (this.activeResult >= 0)) {
        if (document.getElementById('searchResult' + this.activeResult).
          ...
          if (url) {
			//подставляем выбранное слово из списка дополнений в поле поиска
			this.inputFieldEl.value = el.innerHTML;
            this.hideResultsWindow_(); 
            this.goToUrl_(url);
          } 
        } else {
